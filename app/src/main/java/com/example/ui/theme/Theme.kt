...
fun MyApplicationTheme(
    ...
    val effectivePrimary = effectivePalette.getPrimary(isDark, isAmoled)
    val effectiveSecondary = effectivePalette.getSecondary(isDark, isAmoled)
    val effectiveTertiary = effectivePalette.getTertiary(isDark, isAmoled)
    ...
    val colorScheme = when {
        ...
        isAmoled -> {
            ...
            darkColorScheme(
                ...
                background = amoledBg,
                surface = amoledBg,
                surfaceVariant = if (effectivePalette.isTwoColor) Color(0xFF121212) else effectiveSecondary.copy(alpha = 0.14f),
                ...
                onSurfaceVariant = effectivePalette.getNeutralMutedTextColor(isDark = true),
                outline = effectivePalette.getNeutralBorderColor(isDark = true),
                outlineVariant = effectivePalette.getNeutralBorderColor(isDark = true).copy(alpha = 0.35f)
            )
        }
        isDark -> {
            val darkBg = getAppDimBackgroundColor(effectivePrimary, isDark = true, isAmoled = false)
            val darkSurface = Color(0xFF1E1C24)
            val darkSurfaceVariant = if (effectivePalette.isTwoColor) Color(0xFF282531) else effectiveSecondary.copy(alpha = 0.16f)
            ...
            lightColorScheme(
                ...
                background = lightBg,
                surface = Color.White,
                surfaceVariant = if (effectivePalette.isTwoColor) Color(0xFFF1F5F9) else effectiveSecondary.copy(alpha = 0.10f),
                ...
                onSurfaceVariant = effectivePalette.getNeutralMutedTextColor(isDark = false),
